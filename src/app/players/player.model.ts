...
export class Player {
  ...
  private updateWins(scores: number[] | undefined, player2Scores: number[] | undefined) {
    if (!scores || !player2Scores) {
      throw new Error("Scores not defined");

    }
    const won = this.playerWon(scores, player2Scores)
    this.wins = won ? this.wins + 1 : (this.wins || 1) - 1;
  }

  private playerWon(scores: number[], player2Scores: number[]): boolean {
    let player1Wins = 0;
    let player2Wins = 0;
